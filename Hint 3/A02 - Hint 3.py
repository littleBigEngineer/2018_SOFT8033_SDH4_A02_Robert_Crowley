...
# ------------------------------------------
def group(rdd):
  global total, cuisine, average, quarter
  total = 0
  cuisine = 0
  average = 0.0
...
def my_model(ssc, monitoring_dir, result_dir, percentage_f, window_duration, sliding_duration, time_step_interval):
    ...
    windowsDStream = jsonMapDStream.window(window_duration * time_step_interval, sliding_duration * time_step_interval)
    reducedFieldsDStream = windowsDStream.map(lambda x: reduceFields(x))
    ...
    sortedDStream.saveAsTextFiles(result_dir)
